/*jslint
    browser: true
*/
/*global window,console,document,data,metrics*/
...
game.onSecondFlip = async function (x, y) {
    if (game.countFlipped() === 1) {
        ...
        game.showFlipped();
        game.showMatches(); 
        var pairs = game.matchedCardsList.length;
        game.sumTime(duration);
        metrics.updateResults(game.time, pairs); 
    }
}

game.sumTime = function(duration) {
    game.time = game.time + duration;
}
